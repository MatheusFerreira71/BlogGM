...
module.exports = {
    novo: async (req, res) => {
        try {
            await Post.create(req.body); // Post é um modelo, logo podemos usar os métodos do mongoose.
    ...
    listar: async (req, res) => {
        if (Object.keys(req.query).length > 0) {
            ...
            try {
                ...
                const lista = await Post.find().populate('usuario');
    ...
    obterUm: async (req, res) => {
        try {
            ...
            const post = await Post.findById(id); // Encontra a post pelo seu id e retorna um objeto.
            if (post) { // post foi encontrado.
    ...
    atualizar: async (req, res) => {
        try {
            ...
            if (post) { //post encontrado e atualizado
    ...
    excluir: async (req, res) => {
        try {
            ...
            if (post) { //post encontrado e excluída.
                res.status(204).end();// HTTP 204: No content
            } else {
                //Http 404: Not found.
                res.status(404).end();
