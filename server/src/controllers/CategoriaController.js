...
module.exports = {
    index: async (req, res) => {
        try {
            const categorias = await Categoria.find({isSub: false}).sort({titulo: 'asc'});
            const cats = categorias.map(categoria => {
                const { _id, titulo } = categoria;
                return { _id, titulo, subs: [] }
            })
            const subCategorias = await SubCategoria.find().populate({ path: 'catFilha', select: 'titulo' });
            ...
            const parsedCategories = cats.map(categoria => {
                ...
                return (subs.includes(undefined) || subs.length === 0) ? { _id, titulo } : categoria
            }); 
